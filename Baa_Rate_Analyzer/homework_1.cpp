...
void run_unit_tests()
{
    vector<double> test_rates = {5.0, 6.0, 7.0};
    vector<string> test_dates = {"2020-01", "2020-02", "2020-03"};

    bool passed = true;

    if (average(test_rates) != 6.0)
    {
        cout << "average() test failed." << endl;
        passed = false;
    }

    if (find_rate(test_rates, test_dates, "2020-02") != 6.0)
    {
        cout << "find_rate() valid-date test failed." << endl;
        passed = false;
    }

    if (find_rate(test_rates, test_dates, "1999-01") != -1.0)
    {
        cout << "find_rate() invalid-date test failed." << endl;
        passed = false;
    }
